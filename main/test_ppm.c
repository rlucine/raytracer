...
int main(void) {
    ...
    assert(image_Create(&ppm, width, height));
    ...
    for (y = 0; y < height; y++) {
        for (x = 0; x < width; x++) {
            ...
            color.g = (y * 8) % 256;
            color.b = (x + y) % 256;
            assert(image_SetPixel(&ppm, x, y, &color));
    ...
    assert(!image_SetPixel(&ppm, width, height, &color));
    assert(image_GetPixel(&ppm, width, height) == NULL);
    
    // Encode and decode test
    const char *filename = "test_ppm.ppm";
    assert(ppm_Encode(&ppm, filename));
    
    IMAGE decode;
    assert(ppm_Decode(&decode, filename));
